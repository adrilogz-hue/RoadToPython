def pedir_numero(mensaje):
    # Repite la pregunta hasta que el usuario escriba un número entero válido.
    while True:
        try:
...
def pedir_numero_positivo(mensaje):
    # Usa pedir_numero() y además exige que el número sea mayor que 0.
    while True:
...
def crear_inventario():
    # Crea un inventario inicial si todavía no existe inventario.txt.
    return {
...
def guardar_inventario(inventario):
    # Guarda el diccionario inventario dentro de inventario.txt.
...
def cargar_inventario():
    # Intenta leer inventario.txt.
    # Si el archivo no existe, usa un inventario inicial.
    inventario = {}
    ...
    try:
        with open("inventario.txt", "r", encoding="utf-8") as archivo:
            for linea in archivo:
                ...
                # Si hubiera una línea vacía, la ignoramos.
                if linea != "":
                    partes = linea.split(":")
        ...
        print("No existe inventario.txt. Se creará un inventario inicial.")
